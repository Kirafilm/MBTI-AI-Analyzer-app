...
let _supabaseIssuer: string | null = null;

function getSupabaseVerifier() {
  const raw = process.env.SUPABASE_URL ?? process.env.EXPO_PUBLIC_SUPABASE_URL ?? "";
  let supabaseUrl = raw.trim().replace(/\/+$/, "");
  if (!supabaseUrl) return null;

  if (_supabaseJwks && _supabaseIssuer) {
    return { jwks: _supabaseJwks, issuer: _supabaseIssuer };
  }
  ...
  try {
    ...
    if (!path || path === "/" || path.startsWith("/auth/v1") || path.startsWith("/rest/v1")) {
      ...
      supabaseUrl = `${parsed.origin}${path}`;
    }
  } catch {
    return null;
  }
  ...
  _supabaseJwks = jwks;
  _supabaseIssuer = issuer;
  return { jwks, issuer };
}

async function verifySupabaseToken(token?: string): Promise<SupabaseIdentity | null> {
  if (!token) return null;
  const verifier = getSupabaseVerifier();
  if (!verifier) return null;

  try {
    const { payload } = await jwtVerify(token, verifier.jwks, { issuer: verifier.issuer });
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    if (!session) {
      const supabaseIdentity = await verifySupabaseToken(sessionCookie);
      if (!supabaseIdentity) {
        throw ForbiddenError("Invalid session cookie");
      }

      const userId = supabaseIdentity.openId;
